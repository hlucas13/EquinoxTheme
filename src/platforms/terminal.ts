...
import { AnsiPalette, EquinoxVariant } from '../tokens';
...
function hexToTerminalRgb(hex: string): { red: number; green: number; blue: number } {
...
// Map ANSI color index to Terminal.app key
const ansiColorMap: Record<number, keyof AnsiPalette> = {
  0: 'black',
  1: 'red',
  2: 'green',
  3: 'yellow',
...
export function terminalProfileToXml(profile: TerminalProfile): string {
  const { red: cursorR, green: cursorG, blue: cursorB } = hexToTerminalRgb(profile.cursorColor);
  const { red: fgR, green: fgG, blue: fgB } = hexToTerminalRgb(profile.foregroundColor);
  const { red: bgR, green: bgG, blue: bgB } = hexToTerminalRgb(profile.backgroundColor);
  ...
  plist += createColorDict(hexToTerminalRgb(profile.ansiPalette.brightYellow));
  ...
  plist += createColorDict(hexToTerminalRgb(profile.ansiPalette.brightMagenta));
...
function createColorDict(rgb: { red: number; green: number; blue: number }): string {
  let result = '<dict>\n';
